...
--> Block the opponent in the sense that he can no longer move his pawn.
        """
        messagebox.showinfo(title="Game Rules", message=rulesText, detail="by Yann & Raphaël")

    def onLabelClick(self, row, col) -> None:
        if self.playerPlaced < 2:
            self.placePawn(self.game.getPlayer1() if self.playerPlaced == 0 else self.game.getPlayer2(), row, col)
            self.playerPlaced += 1
            self.turnLabel.config(text="Turn: Player {}".format("2" if self.playerPlaced == 1 else "1"))
        else:
    ...
    def updatePlayerTurn(self) -> None:
        self.turnLabel.config(text="Turn: Player {}".format("1" if self.playerToPlay.getPlayer() == 1 else "2"))
